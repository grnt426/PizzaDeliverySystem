...
//				case 9: viewLiveEventHistory(); break;
				case 10:
					...
					break;
				default:
					System.err.println("How did you get here? You " +
	...
	private void cancelOrder() {
		int total_non_canceled =
				controller.getOrderData().getTotalNonCanceledOrders();
		if (total_non_canceled == 0) {
			System.out.println("\t\tThere are no orders to cancel.");
			scan.nextLine();
			return;
		}

		// otherwise present a list of options of orders to cancel
		System.out.println("To choose an order to cancel, type its number.");
		int count = 0;
		for (Order order : controller.getOrderData().getOrders()) {
			System.out.print("\t" + count + ". ID #" + order.getOrderId());
			if (order.isOrderCanceled()) {
				System.out.print(" # Canceled #");
			}
			System.out.println();
		}
		System.out.println("Order to Cancel (0-" + (total_non_canceled - 1) +
				"): ");
		int order_to_cancel = getIntSelect(0, total_non_canceled - 1);
		List<Order> orders = controller.getOrderData().getOrders();
		if (!orders.get(order_to_cancel).isOrderCanceled()) {
			controller.cancelOrder(orders.get(order_to_cancel));
		}
	}
...
//	}
	...
	/**
	 * Logs user out of the CLI.
	 */
	private void killCLI() {
		String answer = null;
		boolean ask = true;
		while (ask) {
			System.out.println("Are you sure you want to Logout? Type yes or no. ");
