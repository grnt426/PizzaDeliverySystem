...
public class CustomerPanel extends JPanel implements ActionListener {
	...
	private Customer cur_select_customer;
	private JPanel cur_select_custs_panel;
	...
	private void initGUI() {
		...
		width = screenDims.width - 200;
		height = screenDims.height - 200;
...
//		this.add(new JLabel("sdfdsf"), BorderLayout.CENTER);
	...
	private void deselectCustomer() {
		if(cur_select_custs_panel != null)
			cur_select_custs_panel.setBackground(Color.WHITE);
		cur_select_custs_panel = null;
		cur_select_customer = null;
	}

	private void addCustomer() {
		controller.launchCustomerModifyGUI();
	}
	...
	public void updateCustomerData() {
		...
		for (final Customer customer : customers) {
			...
			// The panel that was contain all this customer's information
			// and sub-panels
			final JPanel custPanel = new JPanel(new BorderLayout(60, 0));

			// set this panel's properties
			//custPanel.setPreferredSize(new Dimension(width - width / 9, 100));
			custPanel.setMaximumSize(new Dimension(9400, 100));
			...
			MouseListener l = new MouseAdapter() {
				...
				public void mousePressed(MouseEvent e) {
					clickedCustomer(customer, custPanel);
				}
			};
			custPanel.addMouseListener(l);

			// customer information breakdown
			JPanel custInfo = new JPanel(new GridLayout(2, 2));
			custPanel.add(custInfo, BorderLayout.WEST);
			...
			custPanel.add(custOrders, BorderLayout.EAST);
			custOrders.add(new JLabel("Total Orders: "));
			custOrders.add(new JLabel(
					custs_orders.size() + ""));
			custOrders.add(new JLabel("Total Cost: $"));
			custOrders.add(new JLabel(total_spent + ""));

			// finally, add this customer to the list of customers
	...
	private void clickedCustomer(Customer customer, JPanel custs_panel) {
		if (cur_select_custs_panel != null)
			cur_select_custs_panel.setBackground(Color.WHITE);
		custs_panel.setBackground(new Color(154, 176, 198));
		cur_select_custs_panel = custs_panel;
		cur_select_customer = customer;
	}

	private Customer getSelectedCustomer() {
		return cur_select_customer;
	}
